...
GLOBAL_VAR_INIT(round_ext, ROUND_EXTENSION_TIME)
...
SUBSYSTEM_DEF(ticker)
	...
	var/time_until_vote = 90 MINUTES
...
/*					if(failedstarts >= 13)
		...
		if(GAME_STATE_PLAYING)
			...
			check_maprotate()
			var godname
			godname = pick("Zlod", "Sun King", "Gaia", "Aeon", "Gemini", "Aries")
			...
			if(firstvote)
				if(world.time > round_start_time + time_until_vote)
					SSvote.initiate_vote("restart", "[godname]")
...
/datum/controller/subsystem/ticker/proc/checkreqroles()
	...
	if("Merchant" in readied_jobs)
		if(("King" in readied_jobs) || ("Queen" in readied_jobs))
			...
			var/list/stuffy = list("We need... a ruler?!")
			to_chat(world, "<span class='purple'>[pick(stuffy)]</span>")
			return FALSE
	else
		var/list/stuffy = list("Prosper in the Wyrld!")
		to_chat(world, "<span class='purple'>[pick(stuffy)]</span>")
#ifdef DEPLOY_TEST
	var/amt_ready = 999
#else
