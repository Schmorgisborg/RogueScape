	...
	if(SSticker.current_state <= GAME_STATE_PREGAME)
		if(client.ckey in GLOB.hiderole)
			...
			if(client.patreonlevel() >= 1)
				if(alert(src,"Hide your choice of role this round?","ROUGESCAPE","YES","NO") == "YES")
					if(SSticker.current_state <= GAME_STATE_PREGAME)
...
/mob/dead/new_player/verb/donatorupgrade()
	...
	var/amt = input(src, "Donate Triumphs (Gives you donator bonuses for one round)", "ROUGESCAPE") as null|anything in list("10 TRI", "25 TRI", "50 TRI")
	var/added
	var/amt2take = 0
	if(amt == "10 TRI")
		added = 1
...
/mob/dead/new_player/verb/forcerogueworld()
	...
	if(istype(C))
		if(C.allmig || C.roguefight)
			to_chat(src, "<span class='warning'>Too late.</span>")
			return
		if(alert(src,"Force ROGUEWORLD? (30 TRI)","ROUGESCAPE","YES","NO") == "YES")
			if(SSticker.current_state <= GAME_STATE_PREGAME)
				if(get_triumphs() >= 30)
					adjust_triumphs(-30)
...
/mob/dead/new_player/verb/wipetriumphs()
	...
	if(SSticker.current_state > GAME_STATE_PREGAME)
		to_chat(src, "<span class='warning'>Too late.</span>")
		return
	if(alert(src,"Burn down the hall of TRIUMPHS?","ROUGESCAPE","YES","NO") == "YES")
		if(SSticker.current_state <= GAME_STATE_PREGAME)
