		...
		if(bclass == BCLASS_BLUNT || bclass == BCLASS_SMASH || bclass == BCLASS_PICK)
			...
			if(prob(used))
				for(var/datum/wound/fracture/W in simple_wounds)
					return FALSE
				var/list/phrases = list("The skull shatters with a gruesome crunch!", "The skull is fractured!", "The skull caves in!")
				src.next_attack_msg += " <span class='crit'><b>Critical hit!</b> [pick(phrases)]</span>"
				if(prob(33))
					phrases = "The skull caves in with a gruesome crunch!"
					death()
					return TRUE
