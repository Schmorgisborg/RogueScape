...
/mob/living/carbon/human/proc/churchexcommunicate()
	...
	if(inputty)
		...
		if(!found)
			to_chat(src, "<span class='warning'>Something isn't right...</span>")
			return FALSE
		GLOB.excommunicated_players += inputty
		priority_announce("[real_name] has put Psydon's curse of woe on [inputty] for offending the church!", title = "SHAME", sound = 'sound/misc/excomm.ogg')

/mob/living/carbon/human/proc/churchannouncement()
	set name = "Announcement"
	set category = "Monk"
	var/inputty = input("Make an announcement (15 minute cd)", "ROUGESCAPE") as text|null
	if(inputty && isliving(src))
		if(src.mob_timers[MT_ANNOUNCE])
			if(world.time < src.mob_timers[MT_ANNOUNCE])
				to_chat(src, "<span class='warning'>Patience.</span>")
				src.mob_timers[MT_ANNOUNCE] += 1 MINUTES
			else
				src.mob_timers[MT_ANNOUNCE] = world.time + 14 MINUTES
				priority_announce("[inputty]", title = "The Monk Speaks", sound = 'sound/misc/bell.ogg')
		else
			src.mob_timers[MT_ANNOUNCE] = world.time + 14 MINUTES
			priority_announce("[inputty]", title = "The Monk Speaks", sound = 'sound/misc/bell.ogg')
