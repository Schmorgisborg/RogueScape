...
//	dat += "<a href='?_src_=prefs;preference=tab;tab=3' [current_tab == 3 ? "class='linkOn'" : ""]>Keybinds</a>"
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			//villian selection disabled 4/5
...
//			dat += "<b><a href='?_src_=prefs;preference=name;task=random'>Random Name</A></b><BR>"
			...
			dat += "<b>Faith:</b> <a href='?_src_=prefs;preference=faith'>The Divine Ones</a><BR>"
...
// 			-----------END OF IDENT TABLE-----------
			...
			dat += "</td>"
			dat += "<td width=5% valign='top'>"
			// Rightmost column, 40% width
			dat += "<td width=60% valign='top'>"
			dat += "<h2>Body</h2>"
...
/datum/preferences/proc/SetChoices(mob/user, limit = 15, list/splitJobs = list("Master Wizard", "Monk", "Nightman", "Skeleton"), widthPerColumn = 295, height = 620) //295 620
	if(!SSjob)
		return
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("name")
					...
					if(new_name && !sanitize_name(new_name))
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("age")
					var/new_age = input(user, "Choose your character's age (18-[pref_species.max_age])", "Yils Alive") as null|anything in pref_species.possible_ages
					if(new_age)
						age = new_age
						var/list/hairs
						if((age == AGE_OLD) && (OLDGREY in pref_species.species_traits))
				...
				//added 4/4
				if("charflaw")
					var/datum/charflaw/flawinput = input(user, "Choose your character's flaw", "Vices") as null|anything in GLOB.character_flaws
					if(flawinput)
						charflaw = GLOB.character_flaws[flawinput]
						charflaw = new charflaw()
						to_chat(user,"<font color='yellow'>I'm a [charflaw.name]</font>")
						to_chat(user,"<font color='purple'>[charflaw.desc]</font>")

				if("patron")
					var/datum/patrongods/god_input = input(user, "Choose your character's primary deity", "Deities") as null|anything in GLOB.patronlist
					if(god_input)
						selected_patron = god_input
						to_chat(user, "<font color='yellow'>I believe in [selected_patron]</font>")
						to_chat(user, "<font color='teal'>Domain: [selected_patron.domain]</font>")
						to_chat(user, "<font color='teal'>Background: [selected_patron.summary]</font>")
						to_chat(user, "<font color='teal'>Likely Worshippers: [selected_patron.worshippers]</font>")
						ResetJobs()
						to_chat(user, "<font color='red'>Classes reset.</font>")
						...
						if(S)
							for(var/i=1, i<=max_save_slots, i++)
								...
								if(!name)
									name = "Slot[i]"
								choices[name] = i
					var/choice = input(user, "CHOOSE A HERO","ROGUESCAPE") as null|anything in choices
					if(choice)
