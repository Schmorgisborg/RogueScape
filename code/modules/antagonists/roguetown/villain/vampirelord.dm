			...
			if(isturf(H.loc))
				...
				if(T.can_see_sky())
					if(T.get_lumcount() > 0.15)
						if(!isspawn)
							to_chat(H, "<span class='warning'>Saradomin spurns me! I must get out of his rays!</span>") // VLord is more punished for daylight excursions.
							sleep(100)
							var/turf/N = H.loc
							if(N.can_see_sky())
...
/mob/living/carbon/human/proc/demand_submission()
	...
	if(istype(C))
		if(C.kingsubmit)
			to_chat(src, "I am already the Master of the Wyrld.")
			return
		...
		switch(choice)
			...
			if("Steal the Sun")
				...
				if(GLOB.tod == "night")
					...
					return
				if(alert(user, "Force Wyrld into Night? Cost:5000","","Yes","No") == "Yes")
...
/datum/objective/vampirelord/conquer
	name = "conquer"
	explanation_text = "Make the Ruler of Wyrld bow to my will."
