...
//	var/t_him = p_them()
	...
	if(obscure_name)
		if(job)
			var/datum/job/S = SSjob.GetJob(job)
			if(S.title == "Skeleton")
				. = list("<span class='info'>ø ------------ ø</span><span class='userdanger'>\nA <EM>skeleton</EM>, undead plague!</span><span class='info'>")
			else
		...
		if(ishuman(user))
			...
			if(H.PATRON)
				if(H.PATRON != "Zamorak")
					. += "A follower of [H.PATRON]."
				else
					. += "One of [H.PATRON]'s cultists."
