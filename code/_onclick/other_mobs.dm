				...
				if(ishuman(A))
					...
					if(stealroll > targetperception)
						if(U.get_active_held_item())
							to_chat(src, "<span class='warning'>I'm holding something.</span>")
							return
						if(!(zone_selected in stealablezones))
							to_chat(src, "<span class='warning'>Nothing is there.</span>")
							return
						...
						if (length(stealpos) > 0)
							...
							to_chat(src, "<span class='warning'>There was nothing to steal.</span>")
					if(stealroll <= 4)	
						to_chat(V, "<span class='danger'>Someone tried pickpocketing me!</span>")
						src.visible_message("<span class='warning'>[src] fumbles around in [V]'s pockets.</span>")
					else if(stealroll <= targetperception)
