...
/obj/screen/zone_sel/Click(location, control,params)
	...
	if(ismob(hud.mymob))
		...
		if(M.gender == FEMALE)
			...
			choice = get_zone_at(icon_x, icon_y, MALE)
	if (!choice)
		return 1
...
/obj/screen/stress
	name = "sanity"
	icon = 'icons/mob/roguehud.dmi'
	icon_state = "oldmoodback"
	var/state2use = "oldmoodback"
	var/stressholder = 0
...
/obj/screen/stress/update_icon()
	...
	if(ishuman(usr))
		...
		if(H.stress)
			stressholder = H.stress
			switch(H.stress)
				if(1 to 2)
					state2use = "stress2old"
				if(3 to 4)
					state2use = "stress4old"
				if(5 to 6)
					state2use = "stress6old"
				if(7 to 8)
					state2use = "stress8old"
				if(9 to 10)
					state2use = "stress10old"
				if(11 to 12)
					state2use = "stress12old"
				if(13 to 14)
					state2use = "stress14old"
				if(15 to 16)
					state2use = "stress16old"
				if(17 to 18)
					state2use = "stress18old"
				if(19 to 20)
					state2use = "stress20old"
				if(21 to 22)
					state2use = "stress22old"
				if(23 to 24)
					state2use = "stress24old"
				if(25 to 26)
					state2use = "stress26old"
				if(27 to 28)
					state2use = "stress28old"
				if(29 to 32)
					state2use = "stress34old"
		else
